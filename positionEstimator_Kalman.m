function [decodedPosX,decodedPosY] = positionEstimator_Kalman(testData,modelParameters) 
...
%     (eye(size(K,1)) - K * H) * Pbar 
    ...
    if length(testData.spikes)<321 

        decodedPosX = testData.startHandPos(1); 

        decodedPosY = testData.startHandPos(2); 
